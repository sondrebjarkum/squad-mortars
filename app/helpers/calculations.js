 ...
 *
 * @param {L.LatLng} a - base point A
 * @param {L.LatLng} b - target point B
 ...
 * @param {L.LatLng} a - point A
 * @param {L.LatLng} b - point B
...
export function getElevation(dist = 0, vDelta = 0, vel = 0) {
  var padding = 0;
  const GRAVITY = App.config.gravity * App.config.activeWeapon.gravityScale;
  const P1 = Math.sqrt(
    vel ** 4 - GRAVITY * (GRAVITY * dist ** 2 + 2 * vDelta * vel ** 2),
  );

  if (App.config.activeWeapon.name === 'Technical') {
  ...
  return (
    padding +
    Math.atan(
      (vel ** 2 - P1 * App.config.activeWeapon.getAngleType()) /
        (GRAVITY * dist),
    )
  );
}
...
export function getOffsetLatLng(pos) {
  const mapScale =
    App.config.canvas.size /
    Layers.find((elem, index) => index == App.config.activeMap).size;
  return {
    lat:
      (pos.lat +
        Layers.find((elem, index) => index == App.config.activeMap).offset[0] *
          mapScale) *
      mapScale,
    lng:
      (pos.lng +
        Layers.find((elem, index) => index == App.config.activeMap).offset[1] *
          mapScale) *
      mapScale,
  };
}
 ...
 * @returns {number | string} - relative height in meters
 */
export function getHeight(a, b) {
  ...
  Aheight = ctx.getImageData(
    Math.round(AOffset.lat),
    Math.round(AOffset.lng),
    1,
    1,
  ).data;
  Bheight = ctx.getImageData(
    Math.round(BOffset.lat),
    Math.round(BOffset.lng),
    1,
    1,
  ).data;
  ...
  Aheight =
    (255 + Aheight[0] - Aheight[2]) *
    Layers.find((elem, index) => index == App.config.activeMap).scaling;
  Bheight =
    (255 + Bheight[0] - Bheight[2]) *
    Layers.find((elem, index) => index == App.config.activeMap).scaling;
...
function resetCalc() {
  ...
  // $('#errorMsg')
  //   .addClass('pure-u-4-24')
  //   .removeClass('errorMsg')
  //   .removeClass('pure-u-1')
  //   .html('-');
...
export function shoot(inputChanged = '') {
  ...
  heightA = App.minimap.heightmap.getHeight(
    L.latLng([aPos.lng * -mapScale, aPos.lat * mapScale]),
  );
  heightB = App.minimap.heightmap.getHeight(
    L.latLng([bPos.lng * -mapScale, bPos.lat * mapScale]),
  );
  height = heightB - heightA;

  distance = getDist(aPos, bPos);
  bearing = getBearing(aPos, bPos);
  vel = App.config.activeWeapon.getVelocity(distance);
  ...
  if (Number.isNaN(elevation)) {
    showError(
      'Target is out of range : ' + distance.toFixed(0) + 'm',
      'target',
    );
    return 1;
  }

  if (elevation > App.config.activeWeapon.minElevation[1]) {
...
function insertCalc(bearing, elevation, distance, vel, height) {
  ...
  console.log(
    `${$('#mortar-location').val()} -> ${$('#target-location').val()}`,
  );
  console.log(
    `-> Bearing: ${bearing.toFixed(1)}° - Elevation: ${elevation.toFixed(1)}↷`,
  );
  console.log(
    `-> Distance: ${distance.toFixed(0)}m - height: ${height.toFixed(0)}m`,
  );
  ...
  animateCalc(
    $('#elevationNum').html(),
    elevation.toFixed(App.config.activeWeapon.elevationPrecision),
    500,
    'elevationNum',
  );

  $('elevation').html(
    $('<i class="fas fa-drafting-compass fa-rotate-180 resultIcons"></i>'),
  );

  if (App.config.activeWeapon.getAngleType() === -1) {
    $('#highlow').html(
      $('<i class="fa-solid fa-sort-amount-up resultIcons"></i>'),
    );
  } else {
    $('#highlow').html(
      $('<i class="fa-solid fa-sort-amount-down resultIcons"></i>'),
    );
  }

  if (
    App.config.activeWeapon.name != 'mortar' &&
    App.config.activeWeapon.name != 'UB-32'
  ) {
...
export function filterInput(e) {
  ...
  if (chrTyped.match(/\d|[\b]|SPECIAL|[A-Za-z]/)) {
    return true;
  }
...
export function saveCalc() {
  ...
  $('.saved_list').append(
    "<p class='savedrow'>" +
      "<input maxlength=\"20\" spellcheck='false' placeholder='" +
      encodeURI($('#target-location').val()) +
      "'class='friendlyname'></input>" +
      '<span class="savespan"> ➜ ' +
      $('#bearing').text() +
      ' - ' +
      $('#elevation').text() +
  ...
  $('.saved_list p')
    .find('input')
    .last()
    .width($('#target-location').val().length * 1.2 + 'ch');
...
export function copyCalc(e) {
  ...
  if (
    App.config.activeWeapon.name != 'mortar' ||
    App.config.activeWeapon.name != 'B-32'
  ) {
    if (
      $(e.target).hasClass('fa-sort-amount-down') ||
      $(e.target).hasClass('fa-sort-amount-up')
    ) {
      return 1;
    }
  }
...
export function changeHighLow() {
  ...
  App.config.activeWeapon.angleType = isLowAngle ? 'low' : 'high';
  shoot();
}
...
function getVerticalSpread(angle, vel) {
  const moa = degToRad(App.config.activeWeapon.moa / 2 / 60);
  const gravity = App.config.gravity * App.config.activeWeapon.gravityScale;
 ...
 * @param {number} [velocity] - initial mortar projectile velocity in m/s
...
function getProjectilePathDistance(angle, velocity, gravity) {
  ...
  const p2 =
    Math.sin(angle) + Math.cos(angle) ** 2 * Math.atanh(Math.sin(angle));
...
function getHorizontalSpread(angle, velocity, gravity) {
  var MOA = App.config.activeWeapon.moa / 60;
...
export function getTimeOfFlight(angle, vel, heightDiff) {
  ...
  const t =
    vel * Math.sin(angle) +
    Math.sqrt(
      Math.pow(vel, 2) * Math.pow(Math.sin(angle), 2) +
